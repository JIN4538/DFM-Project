...
def cached_load(data,name,unit,confirmed,target,deflection,code_revision):
    return load_model(data,name,unit=unit,dimensions_confirmed=confirmed,
                      target_longest_mm=target,deflection_mm=deflection)
...
with st.sidebar:
    st.subheader("검토할 형상")
    source=st.selectbox("입력",["CAD 기준형상","내 파일","외부 STL 사례"],key="source")
    data=None
    if source=="CAD 기준형상":
        ...
        upload=st.file_uploader("STEP 또는 STL",type=["step","stp","stl"],key="model_upload")
...
if data is None:
    st.info("왼쪽에서 STEP/STL을 선택하세요. 원본 CAD가 없어도 STL의 형상·방향 검토를 시작할 수 있습니다.")
    st.stop()
try:
...
with st.sidebar:
    ...
    with st.form("review_settings"):
        ...
        with st.expander("장비·재료와 검토 기준"):
            ...
            material=st.text_input("재료",value="미확정",key="material")
            slicer=st.text_input("슬라이서·버전",value="미확정",key="slicer")
            wall_limit=st.number_input("최소 벽 검토 기준 (mm, 선택)",min_value=.001,value=None,key="wall_limit")
            hole_limit=st.number_input("최소 홀 검토 기준 (mm, 선택)",min_value=.001,value=None,key="hole_limit")
            basis=st.text_input("기준 출처·시편 기록",value="사용자 탐색 조건; 실물 시편으로 보정하지 않음",key="basis")
            process_notes=st.text_area("공정 조건·설계 요구",placeholder="노즐·온도·속도·냉각·공차·하중·후처리 등",key="notes")
        with st.expander("빌드 공간과 층 설정"):
            use_build=st.checkbox("장비 공간을 검토",key="use_build")
            dims=tuple(st.number_input(f"{axis} 공간 (mm)",min_value=1.,value=250.,key=f"build_{axis}") for axis in "XYZ")
            clearance=st.number_input("각 경계의 여유 (mm)",min_value=0.,value=0.,key="clearance")
            defaults={"MEX":.2,"VPP":.05,"PBF_POLYMER":.1,"PBF_METAL":.03}
            layer=st.number_input("층 높이 (mm)",min_value=.005,max_value=5.,value=defaults[process],format="%.3f",key=f"layer_{process}")
            line=st.number_input("MEX 명목 선폭 (mm)",min_value=.01,max_value=5.,value=.4,key="line_width")
...
if tab=="설계 조치" or tab is None:
    ...
    with c1:
        ...
        if "minimum_mm" in wall["measurements"]:
            st.metric("관측 최소",f"{wall['measurements']['minimum_mm']:.4g} mm")
