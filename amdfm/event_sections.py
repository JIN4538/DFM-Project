...
def inspect_event_sections(mesh, max_samples=8192, *, max_total_segments=1_500_000):
    ...
    result = dict(status="unknown", method="vertex_event_intervals_gauss2/1",
        ...
        envelope_scope="AABB XY area times unresolved height bounds only the omitted-height contribution; it is not a bound on total error, section reconstruction error or manufacturing uncertainty.")
    ...
    for i, (lo, hi) in enumerate(zip(heights[:-1], heights[1:])):
        ...
        interval = dict(index=i, lower_mm=float(lo), upper_mm=float(hi), width_mm=width,
            complete=False, row_indices=[], volume_estimate_mm3=None)
        ...
        result["intervals"].append(interval)
        if runtime_budget:
            unresolved.append(float(heights[-1]-hi))
    ...
    known, missing = math.fsum(pieces), math.fsum(unresolved)
    xy = np.ptp(mesh.vertices[referenced, :2], axis=0)
    result.update(status="complete" if all_complete else "partial" if complete else "unknown",
        ...
        omitted_interval_volume_envelope_mm3=missing*float(np.prod(xy)))
    if all_complete:
