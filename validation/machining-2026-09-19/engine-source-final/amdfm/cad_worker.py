...
def convert(source, destination, deflection):
    from amdfm.cad_features import BoundaryContext, trimmed_analytic_face, MAX_MODEL_BOUNDARY_EDGES
    ...
    face_id = 0
    total_triangles = 0
    boundary_edges_remaining = MAX_MODEL_BOUNDARY_EDGES
    ...
    for body_id, solid in groups:
        ...
        for raw_face in items(solid, TopAbs_FACE):
            ...
            feature.update(trimmed_analytic_face(face, boundary_context))
            features.append(feature)
        boundary_edges_remaining = boundary_context.remaining_edges
