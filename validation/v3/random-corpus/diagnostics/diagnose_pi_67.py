...
with np.load(folder / "model.npz", allow_pickle=False) as data:
    model = Model(trimesh.Trimesh(vertices=data["vertices"], faces=data["faces"], process=False),
        info["metadata"], info["cad_features"], data["face_ids"].copy(), data["body_ids"].copy()).select_body(67)
report = json.loads((folder / "body_067_MEX.json").read_text(encoding="utf-8"))
...
record = dict(source_sha256=model.metadata["source_sha256"], model_fingerprint=model.fingerprint,
    body=67, source_code_sha256=report["provenance"]["code_sha256"],
...
destination = ROOT / "pi-body-67-crossing-diagnosis-01.json"
